...
def find_best_path(visited, current, times, pressure):
    location, time, i = (current[0], times[0], 0) if times[0] > times[1] else (current[1], times[1], 1)
    ...
    for new_location in unvisited:
        ...
        if new_time >= 0:
            ...
            yield from find_best_path(
                visited + [new_location],
                [new_location if i == 0 else current[0], new_location if i == 1 else current[1]],
                [new_time if i == 0 else times[0], new_time if i == 1 else times[1]],
                new_pressure
            )
...
best_pressure = max(find_best_path(["AA"], ["AA", "AA"], [26, 26], 0))
print(best_pressure)
