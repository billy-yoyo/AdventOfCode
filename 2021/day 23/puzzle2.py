...
class Node:
    ...
    def path_to(self, target, length=0, last=None, first=False):
        if self.room_target == target:
            ...
            elif self.values.peek() is None or all(x == self.room_target for x in self.values):
                return self, (length + (3 - len(self.values)))
            else:
                return
        elif self.is_room:
            return None
    ...
    def finished(self):
        return len(self.values) == 4 and all(v == self.room_target for v in self.values)
...
rooms = [
    Node("A", "DDDC"), Node("B", "BCBA"), Node("C", "CBAD"), Node("D", "AACB")
]
